...
//        }
    ...
    public function joininStep1(Request $request)
    {
        ...
        $param['paying_amount']                      = $paying_amount+100;
        $param['sc_bail']                            = 100;
        ...
        if (BModel::getCount('member', ['member_id' => $param['member_id']]) == 0) {
            return Base::jsonReturn(2002, '用户不存在');
        }
    ...
    {
        if (!$data = json_decode(Redis::get('arealists'))) {
            $data = Store::getAreaList();
            Redis::set('arealists', json_encode($data));
        }
