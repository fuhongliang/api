...
//        }
        require_once base_path('lib/aliyun-oss-php-sdk/autoload.php');

// 阿里云主账号AccessKey拥有所有API的访问权限，风险很高。强烈建议您创建并使用RAM账号进行API访问或日常运维，请登录 https://ram.console.aliyun.com 创建RAM账号。
        $accessKeyId = "LTAIHqrjAcTGn5Xe";
        $accessKeySecret = "j4G8mjjWkRD9vKGbi3ugWXZhM76JZw";
// Endpoint以杭州为例，其它Region请按实际情况填写。
        $endpoint = "oss-cn-beijing.aliyuncs.com";
// 存储空间名称
        $bucket = "goods-images-bucket";
        $filename=md5(microtime().rand(0,999));
        $object = $filename.'.'.$image->getClientOriginalExtension();
// <yourLocalFile>由本地文件路径加文件名包括后缀组成，例如/users/local/myfile.txt
        ...
        try {
            ...
            if ($result['info']['http_code'] == 200) {
                return Base::jsonReturn(200, '上传成功', $result['info']['url']);
            } else {
                return Base::jsonReturn(2000, '上传失败');
            }
